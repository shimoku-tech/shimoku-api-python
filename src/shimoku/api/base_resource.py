...
class ResourceCache(ClassWithLogging):
    ...
    async def list(self, limit: Optional[int] = None) -> List[IsResource]:
        ...
        async with self._listing_lock:
            ...
            self._listed = True
            endpoint: str = (
                f'{self._parent["base_url"]}{self._parent.resource_type}/'
                f'{self._parent["id"]}/{self._resource_plural}'
            )

            resources_raw: Dict = await self._parent.api_client.query_element(
                endpoint=endpoint,
                method="GET",
                limit=limit,
    ...
    async def add(
        self,
        resource: Optional[IsResource] = None,
        ...
        if not resource:
            if not alias:
                log_error(
                    logger,
                    "Alias is required when resource is not provided",
                    CacheError,
                )

            resource: IsResource = self._resource_class(
                parent=self._parent, alias=alias
            )
        ...
        if resource.resource_type != self._resource_class.resource_type:
            log_error(
                logger,
                f"Resource {resource_id} has the wrong resource type",
                CacheError,
            )
        ...
        if resource_id in self._cache:
            if self._parent.api_client.cache_enabled:
                log_error(
                    logger,
                    f"Resource {resource_id} already exists in cache",
                    CacheError,
                )
    ...
    async def delete(
        ...
        if not resource:
            log_error(
                logger,
                f"Resource {alias} not found in cache, unable to delete it",
                CacheError,
            )
            return False
        uuid = resource["id"]
...
class BaseResource(ClassWithLogging):
    ...
    def __init__(
        ...
        if parent:
            if not self.parent["id"]:
                log_error(
                    logger,
                    f"Parent resource {self.parent.resource_type} has no id",
                    ResourceIdMissing,
                )
        ...
        if not self.api_client:
            log_error(logger, "Api client has not been defined", ValueError)
    ...
    def get_alias(self, params: Optional[dict] = None) -> Optional[Alias]:
        if self.alias_field:
            if isinstance(self.alias_field, tuple):
                return tuple(
                    self.get_alias_field_value(alias_field, params=params)
                    for alias_field in self.alias_field
                )
            if isinstance(self.alias_field, str):
                return self.get_alias_field_value(self.alias_field, params=params)
            log_error(
                logger,
                f"Alias field {self.alias_field} is not a string or a tuple",
                ValueError,
            )
        else:
            return None
    ...
    async def async_init(self):
        ...
        if self.await_children:
            ...
            await asyncio.gather(
                *[child[1] for c_l in self.children.values() for child in c_l]
            )
    ...
    async def get(self):
        ...
        resource_dict = await self.api_client.query_element(
            method="GET",
            endpoint=endpoint,
        )
        ...
        for field, value in resource_dict.items():
            if field in self.params:
                self.params[field] = (
                    value
                    if field not in self.params_to_serialize
    ...
    async def create(self) -> str:
        ...
        obj = await self.api_client.query_element(
    ...
    async def create_children_batch(
        self,
        resource_class: Type[IsResource],
        children_params: List[Dict],
        unit: str,
        batch_size: int = 100,
    ):
            ...
            query_tasks.append(
                self.api_client.query_element(
                    ...
                    **{
                        ...
                        "progress_bar": (
                            progress_bar,
                            len(children_params[chunk : chunk + batch_size]),
                        ),
                    },
                )
            )
    ...
    async def create_child(
        ...
        if "uuid" in kwargs:
            log_error(
                logger,
                f"Cannot create child with uuid {kwargs['uuid']}, use get instead",
                ValueError,
            )

        if "alias" in kwargs:
            if kwargs["alias"] is None:
                log_error(logger, "Cannot create child with alias None", ValueError)
    ...
    async def update_child(
        ...
        **params,
        ...
        """
        return await self.children[resource_class].update(
            uuid=uuid, alias=alias, **params
        )
    ...
    async def get_child(
        ...
        if not uuid and alias is None:
            log_error(logger, "Cannot get child without uuid or alias", ValueError)
...
class Resource(ClassWithLogging, ABC):
    ...
    def __init__(
        ...
        self._check_params_before_creation = (
            check_params_before_creation if check_params_before_creation else []
        )
        ...
        self.children = self._base_resource.children
        ...
        if not self._base_resource.id:
            for param_name in self._check_params_before_creation:
                if self._base_resource.params[param_name] is None:
                    log_error(
                        logger,
                        f"Mandatory parameter {param_name} has not been defined",
                        ValueError,
                    )
    ...
    def __getitem__(self, item):
        ...
        if item == "id":
            ...
            and item[0] in self._base_resource.params
        ):
            deeper_params = self._base_resource.params[item[0]]
            if item[1] not in deeper_params:
                log_error(
                    logger,
                    f"{self.__class__.__name__} {item[0]} does not contain {item[1]}",
                    KeyError,
                )
            ...
            log_error(
                logger,
                f"{self.__class__.__name__} parameters do not contain {item}",
                KeyError,
            )
    ...
    def __setitem__(self, key, value):
        ...
        if key not in self._base_resource.params:
            log_error(
                logger,
                f"{self.__class__.__name__} parameters do not contain {key}",
                KeyError,
            )
            ...
            if is_dataclass(class_type):
                value = class_type(
                    **{
                        ...
                        if k in class_type.__dataclass_fields__
                    }
                )
            elif not isinstance(self._base_resource.params[key], type(None)):
                log_error(
                    logger,
                    f"Value {value} with type {value.__class__} for parameter {key} "
                    f"is not of type {self._base_resource.params[key].__class__}",
                    ValueError,
                )
