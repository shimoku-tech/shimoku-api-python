...
def get_request_function():
    ...
    if IN_BROWSER_PYODIDE:
        ...
        async def request(
            ...
            session_params = {
                'auth': aiohttp.BasicAuth('user', self.api_key) if self.is_basic_auth else None
            }
            ...
            async with aiohttp.ClientSession(**session_params) as session:
                async with session.request(**request_options) as res:
                    if 'content-type' in res.headers and 'application/json' in res.headers.get('content-type'):
                        data = await res.json()
                    else:
                        data = await res.read()
                    if not res.ok:
...
class ApiClient(ClassWithLogging):
    ...
    def __init__(
        ...
        config=None, server_host='127.0.0.1',
        server_port=8000, retry_attempts: int = 5
    ):
    ...
    async def query_element(
        ...
        element_data: dict = await (
            retry(self.call_api, n_retries=self.retry_attempts)(
                endpoint, method,
                path_params,
                query_params,
                header_params,
    ...
    async def request(
        ...
        if to_tazawa:
            ...
            headers.update({SHIMOKU_VERSION_KEY: get_distribution("shimoku").version
                            if not IN_BROWSER_PYODIDE else '2.0.0'})
