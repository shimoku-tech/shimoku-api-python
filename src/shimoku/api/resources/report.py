...
class Report(Resource):
    ...
    def __new__(
        ...
        if cls is Report:
            if db_resource is None:
                raise ValueError(
                    "You must provide a db_resource to create a Report instance"
                )
            ...
            elif db_resource["reportType"] in [
                "INDICATORS",
                "MULTIFILTER",
                "ECHARTS",
                None,
    ...
    def set_properties(self, **properties):
        ...
        for property_name, value in properties.items():
            if property_name not in self.default_properties.keys():
                raise ValueError(
                    f"Property {property_name} is not a possible property for {self.report_type}"
                )
            if (
                property_name in self.possible_values
                and value not in self.possible_values[property_name]
            ):
                raise ValueError(
                    f"Value {value} is not a possible value for property {property_name}"
                )
    ...
    async def change_report_type(self, report_class: Type["Report"]):
        ...
        logger.info(
            f"Changing component type from {self.report_type} to {report_class.report_type}"
        )
    ...
    async def create_report_data_set(
        self,
        mapping_data_set_sort: tuple[Optional[Mapping], DataSet, Optional[dict]],
        ...
        report_data_set = await self._base_resource.create_child(
            self.ReportDataSet, **params
        )
    ...
    async def delete_report_data_sets(self, log: bool = False):
        ...
        await asyncio.gather(
            *[
                self._base_resource.delete_child(self.ReportDataSet, uuid=rds["id"])
                for rds in report_data_sets
            ]
        )
        if log:
            logger.info(
                f"Deleted {len(report_data_sets)} component data set links from component at {str(self)}"
            )
