...
async def universe_api_key(
    uuid: str = CLIFuncParam(prompt=True),
    **kwargs
):
    ...
    await universes_layer.delete_universe_api_key(uuid=kwargs['uuid'], api_key_uuid=uuid)
...
async def workspace_contents(
    ...
    **kwargs
    ...
    await asyncio.gather(*[
        app_layer.delete_all_menu_path_activities(app['id']) for app in await business.get_apps()
    ])
...
async def board_menu_path_link(
    ...
    **kwargs
    ...
    if not app:
        ...
        return
    await dashboards_layer.remove_menu_path_from_board(menu_path_id=app['id'], uuid=dashboard['id'])
...
async def board_menu_path_links(
    ...
    **kwargs
    ...
    :param board: Board id or name to use
    """
    resource_getter = ResourceGetter(InitOptions(board=board, workspace_id=workspace_id, **kwargs))
    dashboards_layer = await resource_getter.get_dashboards_layer()
    dashboard = await resource_getter.get_dashboard()
    if not dashboard:
        print(f"Board {board} not found")
...
async def activity(
    ...
    with_linked_to_templates: bool = CLIFuncParam(default=False, action='store_true', mandatory=False),
    **kwargs
):
    ...
    if not activity:
        ...
        return
    await activities_layer.delete_activity(uuid=activity['id'], with_linked_to_templates=with_linked_to_templates)
...
async def activities(
    ...
    **kwargs
    ...
    apps_layer = await resource_getter.get_apps_layer()
    app = await resource_getter.get_app()
    await apps_layer.delete_all_menu_path_activities(uuid=app['id'], with_linked_to_templates=with_linked_to_templates)
...
async def file(
    ...
    **kwargs
    ...
    if not file:
        ...
        return
    print(file['id'])
    await files_layer.delete_file(uuid=file['id'], with_shimoku_generated=with_shimoku_generated)
...
async def files(
    ...
    **kwargs
    ...
    apps_layer = await resource_getter.get_apps_layer()
    app = await resource_getter.get_app()
    await apps_layer.delete_all_menu_path_files(uuid=app['id'], with_shimoku_generated=with_shimoku_generated)
...
async def role(
    ...
    **kwargs
    ...
    """
    resource_getter = ResourceGetter(InitOptions(
        workspace_id=workspace_id, menu_path=menu_path, board=board, role=role, **kwargs
    ))
