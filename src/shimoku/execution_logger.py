...
def logging_before_and_after(logging_level: Callable, name: Optional[str] = None) -> Callable:
    ...
    def __getattribute__(self, item):
        ...
        if callable(attr) and not isclass(attr) and logger.isEnabledFor(logging.INFO):
            ...
            if logger.isEnabledFor(logging.DEBUG):
                return logging_before_and_after(logging_level=self._module_logger.debug, name=item)(attr)
        return attr
