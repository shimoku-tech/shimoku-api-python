import unittest
from test_plot_api import main
...
def get_report_data_set_info(
    ...
    report_data_set["dataSetId"] = data_sets_mapping[
        report_data_set["dataSetId"]
    ].replace(report_id, r_hash)
...
def get_report_data_set_info_dict(
    r_hash: str,
    _dict: dict,
    report_data_sets: dict[str, dict],
    data_sets_mapping: dict[str, str],
):
    for k, v in _dict.items():
        if isinstance(v, dict):
            get_report_data_set_info_dict(
                r_hash, v, report_data_sets, data_sets_mapping
            )
        elif isinstance(v, list):
            get_report_data_set_info_list(
                r_hash, v, report_data_sets, data_sets_mapping
            )
        elif isinstance(v, str) and v.startswith("#{") and v.endswith("}"):
            uuid = v[2:-1]
            _dict[k] = get_report_data_set_info(
                r_hash, report_data_sets[uuid], data_sets_mapping
            )
...
def get_report_data_set_info_list(
    ...
    for i, v in enumerate(_list):
        if isinstance(v, dict):
            ...
            uuid = v[2:-1]
            _list[i] = get_report_data_set_info(
                r_hash, report_data_sets[uuid], data_sets_mapping
            )
...
def change_ids_in_tabs_or_modal(component, components_by_id):
    if "tabs" in component["properties"]:
        for tab_name, tab_dict in component["properties"]["tabs"].items():
            component["properties"]["tabs"][tab_name]["reportIds"] = sorted(
                [
                    components_by_id[r_id]["properties"]["hash"]
                    for r_id in tab_dict["reportIds"]
                ]
            )
    if "reportIds" in component["properties"]:
        component["properties"]["reportIds"] = sorted(
            [
                components_by_id[r_id]["properties"]["hash"]
                for r_id in component["properties"]["reportIds"]
            ]
        )
...
def handle_events(events, components_by_id):
    ...
    for event in on_click_events:
        if "modalId" in event["params"]:
            event["params"]["modalId"] = components_by_id[event["params"]["modalId"]][
                "properties"
            ]["hash"]
...
def handle_component(
    shimoku_client,
    component,
    mpath_components_by_id,
    data_sets_mapping,
    current_bentobox,
):
    data_set_links = shimoku_client.components.get_component_data_set_links(
        component["id"]
    )
    get_report_data_set_info_dict(
        component["properties"]["hash"],
        component["properties"],
        {rds["id"]: rds for rds in data_set_links},
        data_sets_mapping,
    )
...
def handle_menu_path(
    shimoku_client, menu_path, components, data_sets_mapping, data_by_data_set
):
    shimoku_client.set_menu_path(menu_path["name"], dont_add_to_dashboard=True)
    mpath_components = sorted(
        shimoku_client.menu_paths.get_menu_path_components(menu_path["id"]),
    ...
    for data_set in data_sets:
        for comp_id in components_hash_by_id.keys():
            if comp_id in data_set["name"]:
                data_set["name"] = data_set["name"].replace(
                    comp_id, components_hash_by_id[comp_id]
                )
        data_sets_mapping[data_set["id"]] = data_set["name"]

    for data_set in sorted(data_sets, key=lambda x: x["name"]):
        data_by_data_set[data_set["name"]] = shimoku_client.data.get_data_from_data_set(
            data_set["id"]
        )
...
def get_workspace_contents(shimoku_client):
    ...
    menu_paths = sorted(
        shimoku_client.workspaces.get_workspace_menu_paths(shimoku_client.workspace_id),
        key=lambda x: x["order"],
    )
    components = []
    data_sets_mapping = {}
    ...
    _all = [
        ...
        *menu_paths,
        *components,
        *[datum for data in data_by_data_set.values() for datum in data],
    ]
    for element in _all:
        element.pop("id")
...
def create_bar_chart(shimoku_client):
    data = [
        {"date": dt.date(2021, 1, 1), "x": 50000000, "y": 5},
...
class TestCodeGen(unittest.TestCase):
    def setUp(self):
        self.shimoku_client = initiate_shimoku()
        self.workspace_id = getenv("BUSINESS_ID")
        self.shimoku_client.set_workspace(self.workspace_id)

    def test_code_gen(self):
        if not self.shimoku_client.playground or mock:
            # It would take too long to complete the test
            return
        self.shimoku_client.set_workspace()
        ...
        boards, menu_paths, components, data = get_workspace_contents(
            self.shimoku_client
        )
        ...
        (
            ...
            first_generation_data,
        ) = get_workspace_contents(self.shimoku_client)
        ...
        results = {
            "orig-first": (
                get_diff_percentage(boards, first_generation_boards),
            ...
            "orig-second": (
                get_diff_percentage(boards, second_generation_boards),
            ...
            "first-second": (
                get_diff_percentage(first_generation_boards, second_generation_boards),
                get_diff_percentage(
                    first_generation_menu_paths, second_generation_menu_paths
                ),
                get_diff_percentage(
                    first_generation_components, second_generation_components
                ),
        ...
        assert all(
            all([value == 0 for value in results_list])
            for results_list in results.values()
        )

    def test_commit(self):
        if self.shimoku_client.playground or mock:
        ...
        local_shimoku_client.pop_out_of_board()
        (
            local_boards,
            local_menu_paths,
            local_components,
        ...
        subprocess.run(
            [
                ...
                "local",
                "--target-access-token",
                self.shimoku_client.access_token,
                "--target-universe-id",
